...
public class TC12_GetSingleResource extends TestClass {
	
	@BeforeClass
	void GetLISTofUsers() throws InterruptedException
	{
		logger.info("TC12_GetSingle<Resource> Test case to be executed");
	...
	void CheckContentType()
	{
	  ...
	  void verifyJsonBodyData()
	  {
		 ...
		 /* int ID=JsonPathEvalutor.get("id");
		  ...
		  String Name=JsonPathEvalutor.get("data.name");
		  logger.info("Name is:==>" +Name);
		  Assert.assertEquals(Name, "fuchsia rose"); 
		  
		  int Year=JsonPathEvalutor.get("data.year");
		  logger.info("The Year is==>" +Year);
		  Assert.assertEquals(Year, 2001); 
		  
		  String value=JsonPathEvalutor.get("data.pantone_value");
		  logger.info("pantone_value is:==>" +value);
		  Assert.assertEquals(value, "17-2031"); 
		  
		  String Color=JsonPathEvalutor.get("data.color");
		  logger.info("Colur is:==>" +Color);
